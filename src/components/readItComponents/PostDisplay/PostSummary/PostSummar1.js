...
import {deletePost, editPost, getPost, votePost} from "../../../../actions/readitActions/postActions";
...
class PostSummary1 extends Component {
    ...
    onSubmitEditPost = (value) => {
        ...
        this.props.history.push(`/portfolio/${slug('Web Apps')}/${slug('ReadIt! (AWS Serverless, React, Redux)')}/${updatedPost.category}/${updatedPost.postId}`)
                                    ...
                                    </IconButton>
                                    {
                                        (this.props.authUser === this.props.post.author)
                                            ? <IconButton onClick={() => this.onDeleteClick(this.props.post.postId,this.props.post.userId)}>
                                                <Link to={`/portfolio/${slug('Web Apps')}/${slug('ReadIt! (AWS Serverless, React, Redux)')}/${this.props.post.category}`}>
                                                    <Delete/>
                                                </Link>
                                            </IconButton>
                                            : null
